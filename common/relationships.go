...
func (r Relationships) AddAutoRelationship(dt office.DocType, src string, idx int, ctype string) Relationship {
	return r.AddRelationship(office.RelativeFilename(dt, src, ctype, idx), ctype)
}

// AddRelationship adds a relationship.
func (r Relationships) AddRelationship(target, ctype string) Relationship {
	if !strings.HasPrefix(ctype, "http://") {
		office.Log("relationship type %s should start with 'http://'", ctype)
	}
...
func (r Relationships) AddHyperlink(target string) Hyperlink {
	rel := r.AddRelationship(target, office.HyperLinkType)
