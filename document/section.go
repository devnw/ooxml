...
func (s Section) SetHeader(h Header, t wml.ST_HdrFtr) {
	...
	hdrID := s.d.docRels.FindRIDForN(h.Index(), office.HeaderType)
	if hdrID == "" {
...
func (s Section) SetFooter(f Footer, t wml.ST_HdrFtr) {
	...
	hdrID := s.d.docRels.FindRIDForN(f.Index(), office.FooterType)
	if hdrID == "" {
...
func (s Section) SetPageMargins(top, right, bottom, left, header, footer, gutter measurement.Distance) {
	...
	margins.TopAttr.Int64 = office.Int64(int64(top / measurement.Twips))
	margins.BottomAttr.Int64 = office.Int64(int64(bottom / measurement.Twips))
	margins.RightAttr.ST_UnsignedDecimalNumber = office.Uint64(uint64(right / measurement.Twips))
	margins.LeftAttr.ST_UnsignedDecimalNumber = office.Uint64(uint64(left / measurement.Twips))
	margins.HeaderAttr.ST_UnsignedDecimalNumber = office.Uint64(uint64(header / measurement.Twips))
	margins.FooterAttr.ST_UnsignedDecimalNumber = office.Uint64(uint64(footer / measurement.Twips))
	margins.GutterAttr.ST_UnsignedDecimalNumber = office.Uint64(uint64(gutter / measurement.Twips))
