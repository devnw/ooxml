...
func TestCreatorUnknownType(t *testing.T) {
	el, err := office.CreateElement(xml.StartElement{Name: xml.Name{Local: "foo", Space: "bar"}})
	if el == nil || err != nil {
		t.Errorf("CreateElement should never return nil: %s", err)
	}
	if _, ok := el.(*office.XSDAny); !ok {
		t.Errorf("CreateElement should return XSDAny for unknown types")
	}
}

func TestCreatorKnownType(t *testing.T) {
	el, err := office.CreateElement(xml.StartElement{Name: xml.Name{Local: "CT_Settings", Space: "http://schemas.openxmlformats.org/wordprocessingml/2006/main"}})
	if el == nil || err != nil {
		t.Errorf("CreateElement should never return nil: %s", err)
	}
